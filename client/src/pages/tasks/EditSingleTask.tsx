...
import { useTaskById, useUpdateTask } from "../../api/tasks/tasks"; // Updated hook
...
const EditSingleTask = () => {
  ...
  const { data: taskData } = useTaskById(taskId);
  ...
  const updateTaskMutation = useUpdateTask(); // Updated hook
  ...
  useEffect(() => {
    if (taskData) {
      ...
      setUserIds(taskData.Users?.map((user) => user.id) || []);
      setProjectId(taskData.projectId);
      setClientId(taskData.clientId);
    }
  }, [taskData]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateTaskMutation.mutate(
      { taskId, data: updatedTaskData }, // Updated mutation payload
      {
        onSuccess: () => {
